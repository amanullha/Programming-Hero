...
const LogIn = () => {
    ...
    const [isRegisterd, setIsRegisterd] = useState(1);
    ...
    const [isToist, setIsToist] = useState(false);
    const [toistMsg, setToiseMgs] = useState("success");
    const [isToistVisible, setIsToistVisible] = useState(false);
    ...
    const handleEmailBlur = (e) => {
        ...
        if (emailText.length !== 0 && validateEmail(emailText) === true) {
            setEmailValid(true);
            setEmailError("Email is good!");
            setEmailColor("green");
            ...
            setEmailValid(false);
            setEmailError("Please a your valid email");
            setEmailColor("red");
    ...
    const verifyEmail = () => {
        ...
        sendEmailVerification(auth.currentUser)
            .then(() => {
                console.log("veryFied email send");
            });
    }
    ...
    const handleCreateAccount = () => {
        ...
        console.log('created');
        if (!passwordValid || !emailValid || !nameValid) return;
        ...
        createUserWithEmailAndPassword(auth, email, password)
            .then((userCredential) => {
                ...
                // go to login page
                setIsRegisterd(isRegisterd ^ 1);

                // back to home after logedIn
                // navigate('/')

                // back to previous page after logedIn
                // navigate('-1')
    ...
    const toistSuccess = () => {
        ...
        setIsToistVisible(true);
        setToiseMgs("Successful")
        setIsToist(true)
        toistInterval();
    }
    const toistFail = () => {
    ...
    return (
        <>
            <div style={isToist ? { color: "green" } : { color: "red" }} className='relative '>
                <p style={
                    isToistVisible ? { display: "block" } : { display: "none" }
                }
                    className={`custom-toast right-2 top-3 absolute bg-slate-200 px-3 rounded-md `}
                > {toistMsg}</p>
            </div>

            {
                isRegisterd ?
                    <>
                        ...
                        <div className="h-full bg-gradient-to-tl from-green-400 to-indigo-900 w-full py-5 px-4">

                            <div className="flex flex-col items-center justify-center">


                                <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
                                    <p tabIndex="0" className="focus:outline-none text-2xl font-extrabold leading-6 text-gray-800">Login here</p>
                                    <p tabIndex="0" className="focus:outline-none text-sm mt-3 font-medium leading-none text-gray-500">Don't have account?
                                        <button onClick={handleRegistered} className=' my-0 text-lg font-bold ml-2 text-black hover:underline'>SignUp here</button>
                                    </p>

                                    <div>
                                        <label id="email" className="text-sm font-medium leading-none text-gray-800 mb-0">
                                            Email
                                        </label>
                                        <input onBlur={handleEmailBlur} aria-labelledby="email" type="email" className="bg-gray-200 border rounded  text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" placeholder='Enter your Email here...' />

                                        <small style={{ color: `${emailColor}` }} className='m-0 p-0 ' > {emailError}</small>
                                    </div>
                                    ...
                                    <div className="  w-full" >
                                        ...
                                        <div className="relative flex items-center justify-center">
                                            ...
                                            <input onBlur={handlePasswordBlur} id="pass" type="password" className="bg-gray-200 border rounded  text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" placeholder='Enter your Password here..' />
                                    ...
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                        ...
                                        <small style={{ color: `${passwordColor}` }} className='pl-1'>{passwordError}</small>
                                    ...
                                    <div className="mt-8">
                                        <button role="button" className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full" onClick={handleLoginAccount}>LogIn</button>
                                    ...
                                    <button onClick={signInWithGoogle} aria-label="Continue with google" role="button" className="focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-gray-700 py-3.5 px-4 border rounded-lg border-gray-700 flex items-center w-full mt-1">
                    ...
                    <>
                        ...
                        <div className="h-full bg-gradient-to-tl from-green-400 to-indigo-900 w-full py-5 px-4">
                            ...
                            <div className="flex flex-col items-center justify-center">
                                ...
                                <div className="bg-white shadow rounded lg:w-1/3  md:w-1/2 w-full p-10 mt-16">
                                    <p tabIndex="0" className="focus:outline-none text-2xl font-extrabold leading-6 text-gray-800">Register Your Self</p>
                                    <p tabIndex="0" className="focus:outline-none text-sm mt-3 font-medium leading-none text-gray-500">Do you have an account?
                                        <button onClick={handleRegistered} className=' my-0 text-lg font-bold ml-2 text-black hover:underline'>Sign In here</button>
                                    </p>
                                    ...
                                    <div>
                                        <label id="name" className="text-sm font-medium leading-none text-gray-800 mb-0">
                                            Full Name
                                        </label>
                                        <input onBlur={handleNameBlur} aria-labelledby="name" type="text" className="bg-gray-200 border rounded  text-xs font-medium leading-none text-gray-800 py-3 w-full pl-3 mt-2" placeholder='Enter your full name here..' />

                                        <small style={{ color: `${nameColor}` }} className='m-0 p-0 ' > {nameError}</small>
                                    </div>
                                    ...
                                    <div className="mt-8">
                                        <button role="button" className="focus:ring-2 focus:ring-offset-2 focus:ring-indigo-700 text-sm font-semibold leading-none text-white focus:outline-none bg-indigo-700 border rounded hover:bg-indigo-600 py-4 w-full" onClick={handleCreateAccount}>Create my account</button>
